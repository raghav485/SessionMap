...
function pushImport(imports: ParsedImport[], specifier: string, symbols: string[], kind: ParsedImport["kind"]): void {
  imports.push({
    specifier,
    symbols,
    kind,
...
export function extractJavaScript(sourceText: string, source: ExtractorSource): ParseResult {
  ...
  for (const match of sourceText.matchAll(/^\s*export\s+(?:const|let|var)\s+([A-Za-z_$][\w$]*)/gmu)) {
    exports.add(match[1]);
    declarations.add(match[1]);
  }
  ...
  if (/^\s*export\s+default\b/gmu.test(sourceText) || /\bmodule\.exports\s*=/gmu.test(sourceText)) {
    exports.add("default");
  }
  for (const match of sourceText.matchAll(/\bexports\.([A-Za-z_$][\w$]*)\s*=/gmu)) {
    exports.add(match[1]);
  }
  ...
  for (const match of sourceText.matchAll(/^\s*(?:const|let|var)\s+([A-Za-z_$][\w$]*)/gmu)) {
    declarations.add(match[1]);
  }
